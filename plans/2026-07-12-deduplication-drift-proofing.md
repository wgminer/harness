...
- [ ] Sync-merge serialization: Swift `.prettyPrinted + .sortedKeys` vs TS `JSON.stringify(x, null, 2)` (insertion order); messages-dedup stamp differs too → merged bytes and revision hashes differ across platforms (`syncMerge.ts` / `sync_merge.rs` / `SyncMerge.swift`). Fix has migration implications (revision hashes change once).
