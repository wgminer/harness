...
/**
 * Legacy `namespace:methodName` IPC name → Rust command name. Tauri matches
 * the invoke name against the snake_case Rust fn identifier verbatim
 * (`#[command(rename_all = "camelCase")]` only renames arguments).
 */
export function legacyIpcCommand(name: string): string {
  return name.split(":").flatMap(words).join("_");
}

/** Legacy `namespace:eventName` → kebab-case event name emitted by Rust. */
export function legacyIpcEvent(name: string): string {
